def main():
    ...
    for x,ar in enumerate(top):
        for y,el in enumerate(ar):
            if el == 0:
                to_reach = set()
                total += do_lookup(x,y, top, 1, to_reach)
    print(total)


def do_lookup(x: int, y: int, top: list[list[int]], num: int, to_reach: set[tuple[int,int]]) -> int:
    if (x,y) in to_reach:
        return 0
    if num > 9:
        return 1
    ...
    for p in pos:
        ...
        if px < 0 or px >= len(top) or py < 0 or py >= len(top[0]):
            continue
        if top[px][py] == num:
            total += do_lookup(px, py, top, num + 1, to_reach)
            if num == 9:
                to_reach.add((px, py))
    return total
